...
export const MY_STORIES = [
  {
    id: 1,
    title: "Life-Changing Technology: 기술이 삶을 바꾸는 방식에 대하여",
    content:
      "저는 기술과 비즈니스의 결합이 인간의 삶을 크게 변화시키는 거대하고 본질적인 힘을 가졌다고 믿습니다. 고도화된 정보 통신 및 처리 기술을 기반으로 구현된 숏폼 비디오가 전 세대의 일상을 점유해버린 것처럼 말입니다. 하지만 이러한 변화가 '우리의 삶을 진정으로 풍요롭게 만들었는가'라는 본질적인 고민은 기술의 발전과 확산 속도에 비해 간과되어 왔다고 생각합니다. 바로 이 지점에서 저는 기술을 통해 사람들의 일상을 실질적으로 개선하고, 삶에 **'긍정적인 임팩트'**를 줄 수 있는 제품을 만드는 데 기여하고 싶다는 갈증을 갖게 되었습니다.",
  },
  {
    id: 2,
    title: "The Bridge: 거시적 관점에서 몰입의 영역으로",
    content:
      "실질적이고 긍정적인 임팩트를 줄 수 있는 프로덕트에 대한 갈증은 저를 새로운 기술을 다루는 비즈니스의 집합체인 벤처캐피탈(VC), 그리고 기업의 실질적인 생존과 성장 전략을 수립하는 전략 컨설팅의 세계로 이끌었습니다. 그곳에서 저는 시장의 흐름을 읽는 법과 비즈니스 메커니즘을 체득하며 논리적 사고의 틀을 다졌습니다. 하지만 지표와 환경을 분석하는 일보다 저를 더 움직였던 것은, '이 기술이 구체적으로 사람들의 일상에 어떤 긍정적인 가치를 가져다주는가?'에 대한 물음이었습니다. 분석가로서 비즈니스에 제언하는 것을 넘어, 사람들의 손에 직접 닿는 가치를 설계하고 구현하고 싶다는 확신이 들었습니다.",
  },
  {
    id: 3,
    title: "Synthesis: UX 디자인, 비즈니스 전략, 엔지니어링",
    content:
      "대학 시절 UX 디자인 전공과 다양한 콘텐츠팀 인턴 활동을 통해 쌓아온 '사용자 중심 사고', 그리고 컨설팅 현장에서 체득한 **'전략적 문제 해결 능력'**은 개발자로서 저의 가장 차별화된 핵심 역량이 되었습니다. (주)스마트팩토리에서 PAL(Process Asset Library) 및 ITSM(IT Service Management) 솔루션 도입 프로젝트를 리드하고, 확장 기능(Add-on) 개발 및 시스템 업그레이드를 직접 수행하며 복잡한 비즈니스 로직을 견고한 시스템으로 구현하는 실무 역량을 탄탄히 다졌습니다. 이제는 이러한 기술적 기반 위에, 사용자의 일상에 자연스럽게 스며들어 감동을 주는 인터페이스를 설계하고 구현하는 프로덕트 엔지니어가 되고자 합니다.",
  },
  {
    id: 4,
    title: "Vision: 프로덕트 엔지니어로서의 지향점",
    content:
      "저는 개발 자체를 진심으로 즐기는 엔지니어이지만, 저의 시선은 단순한 코드 작성을 넘어 그 코드가 사람들에게 제공하는 **'실질적인 가치'**에 머물러 있습니다. 비즈니스의 목표를 명확히 이해하고, 현실적인 기술적 제약 안에서도 최선의 사용자 경험을 도출해 내는 **'프로덕트 엔지니어'**를 지향합니다. 사용자의 수요와 기술의 잠재 가치를 개발 요구사항으로 구체화하는 기획적 안목, 복잡한 문제를 명료하게 구조화하는 전략적 논리력, 그리고 이를 견고한 제품으로 형상화하는 엔지니어링 역량을 하나로 결합하겠습니다. 단순히 기능이 동작하는 것을 넘어, 제가 만든 서비스가 사람들의 일상을 더 나은 방향으로 바꾸는 것이 제가 개발자로서 이루고 싶은 목표입니다.",
  },
];
